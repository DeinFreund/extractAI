 ...
 * @author user
 */
public class EnemyThreat implements IDrawer {
    
    OOAICallback callback;
    zkai zkai;
    
    public EnemyThreat(zkai callback) {
    ...
    public BufferedImage draw(){
        ...
        for (Unit u : callback.getEnemyUnits()){
            ...
            if (threat > 1.0f || threat < 0f){
                zkai.debug("Threat: " + threat + " = " + getDPS(u.getDef()) + " / " + Math.max(u.getHealth(), 50f) + " / " + maxDps);
            }
